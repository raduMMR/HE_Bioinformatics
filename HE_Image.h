...
class HE_Image
{
	...
	*/
	int kernel_size;
public:
	HE_Image(EncryptionParameters &parms);
	...
	void decrypt_after_filtering(int **&matrix_pixels, int N, vector<BigPoly> enc_image,
		int slot_count, BigPoly &secret_key)const;

	void encode_filter(int** &kernel, int N, vector<BigPoly> filter, 
		int slot_count, BigPoly& public_key)const;
