#include "Hom_Conv.h"
#include "_Hom_Convolution\basic_examples.h"
#include <assert.h>
#include "_Hom_Convolution\MyTimer.h"
#include <time.h>
...
void Hom_Conv::filter_signal(vector<BigPoly> &in, vector<BigPoly> &out)const
{
	...
	for (int i = 0; i < in.size(); i++)
	{
		...
		{
			BigPoly ip = evaluator.multiply_plain(in[i], filter[j]);
...
/************************************************************************/
		cout << "Setare filtru incorecta." << endl;
...
void Hom_Conv::PF_filter_signal(vector<BigPoly> &in, vector<BigPoly> &out)const
{
	...
	for (int i = 0; i < in.size(); i++)
	{
		for (int j = 0; j < filter.size(); j++)
		{
			BigPoly ip = evaluator.multiply_norelin(in[i], filter[j]);
			intermediate_products[i + j].push_back(ip);
		}
	}
...
void test_Hom_Conv(const char *log_file, int N, int M)
{
	MyTimer timer;
	ofstream out(log_file, ios::out|ios::app);
	...
	for (int i = 0; i < 2; i++)
	{
		...
		vector<int> signal(M, 1);
		srand(time(NULL));
		for (int i = 0; i < signal.size(); i++)
		{
			signal[i] = rand() % 256 - 128;
		}
...
void test_hom_SIMD(const char *log_file, int N, int M)
{
	MyTimer timer;
	ofstream out(log_file, ios::out | ios::app);
...
void test_hom_PF(const char *log_file, int N, int M)
{
	MyTimer timer;
	ofstream out(log_file, ios::out | ios::app);
...
void test_hom_PF_SIMD(const char *log_file, int N, int M)
{
	MyTimer timer;
	ofstream out(log_file, ios::out | ios::app);
